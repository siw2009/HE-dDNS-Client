...
{
    public partial class TargetSettings : Form
    {
        ...
        {
            this.UpdateArgs();
            this.TargetForm.Text = this.args[0];
            this.TokenForm.Text = this.args[1];
        }


        private void OKButton_Click(object sender, EventArgs e)
        {
            string newTarget = this.TargetForm.Text + "\0";
            string newToken = this.TokenForm.Text + "\0";
            ...
            for (int i = 0; i < newTarget.Length && i < Global.CONFIG_ARGUMENTS_LENGTH; i++) { newConf[i] = (byte)newTarget[i]; }
            for (int i = 0; i < newToken.Length && i < Global.CONFIG_ARGUMENTS_LENGTH; i++) { newConf[i + Global.CONFIG_ARGUMENTS_LENGTH] = (byte)newToken[i];  }
